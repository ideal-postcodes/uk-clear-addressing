...
class AddressModel {
	...
	formattedAddress() {
		...
		if (!isEmpty(this.po_box)) return po_box(this);

		const has_BuildingNumber = !isEmpty(this.building_number);
		const has_BuildingName = !isEmpty(this.building_name);
		const has_SubBuildingName = !isEmpty(this.sub_building_name);
